...
Get mansus image replaced with dll modding

make a default lever entry for cryonics so you can play cryonics legacy without issues that way

Update final boss ai
* Make the combat make sense. Allow you to send followers to attack the foe. Allow use of weapons against Berserkers (longs in the original game)

Fix balance of late game

Remove unnecessary bloat, particularly when in game code is replicated for minimal reason.

Implement capers from the beginning on all legacies
